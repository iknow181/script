...
def flatten_nested_fields(d, parent_key=''):
    """递归地展开字典中的嵌套字段"""
    items = []
    for k, v in d.items():
        new_key = f"{parent_key}_{k}" if parent_key else k
        if isinstance(v, dict):
            items.extend(flatten_nested_fields(v, new_key).items())  # 递归展开子字典
        elif isinstance(v, list):
            # 如果是列表，将列表中的元素逐一展开为字段
            for i, sub_item in enumerate(v):
                if isinstance(sub_item, dict):
                    items.extend(flatten_nested_fields(sub_item, f"{new_key}_{i}").items())
                else:
    ...
    with open(json_path, 'rb') as f:
        for idx, item in enumerate(ijson.items(f, 'item')):
            if isinstance(item, dict):
                # 展开所有嵌套字段
                flat_item = flatten_nested_fields(item)
...
# 示例用法
json_file = "account.json"        # 替换为你的 JSON 路径
output_dir = "./excel"              # 输出目录

os.makedirs(output_dir, exist_ok=True)
json_to_csv_chunks(json_file, output_dir, batch_size=5000, progress_step=1000)
