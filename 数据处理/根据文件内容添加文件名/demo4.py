...
def unique_id_from_path(path: Path):
    """根据文件路径生成8位唯一ID"""
    h = hashlib.md5(str(path.resolve()).encode('utf-8')).hexdigest()
    return h[:8]
    ...
    for root, _, files in os.walk(root_dir):
        for fname in files:
            ...
            try:
                ...
                uid = unique_id_from_path(path)
                new_base = f"{summary}_{uid}"
                new_path = path.with_name(f"{new_base}{path.suffix.lower()}")

                if new_path.exists():
                    log_func(f"[已存在] {new_path.name}（但已自动唯一）")
                    fail += 1
                    continue
